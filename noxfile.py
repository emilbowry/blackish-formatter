...
def _update_npm_packages(session: nox.Session) -> None:
	...
	if (
		package_json["engines"]["vscode"] != package_json["devDependencies"]["@types/vscode"]
	):
		print(
...
def lint(session: nox.Session) -> None:
	...
	session.run("isort", "--profile", "monochromatic", "--check", "./bundled/tool")
	session.run(
		"isort", "--profile", "monochromatic", "--check", "./src/test/python_tests"
	)
	...
	session.run("monochromatic", "--check", "noxfile.py")
